...
def on_predict_start(predictor: object, persist: bool = False) -> None:
    ...
    # Directly load the tracker config from the YAML file specified in predictor.args.tracker
    cfg = IterableSimpleNamespace(**yaml_load(predictor.args.tracker))
    if cfg.tracker_type not in {"bytetrack"}:
    ...
    for _ in range(predictor.dataset.bs):
        ...
        if predictor.dataset.mode != "stream":  # only need one tracker for non-stream modes.
            break
...
def register_tracker(model: object, persist: bool) -> None:
    ...
    Examples:
        Register tracking callbacks to a model:
        >>> model = MyModel()
